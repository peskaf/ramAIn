...
def poly_bg(spectrum: np.ndarray, x_axis: np.ndarray, degree: int, ignore_water: bool = True) -> np.ndarray:
    """
...
def poly(spectral_map: np.ndarray, x_axis: np.ndarray, degree: int, ignore_water: bool) -> np.ndarray:
    """
    ...
    backgrounds = np.apply_along_axis(poly_bg, 2, spectral_map, x_axis, degree, ignore_water)
