from PySide6.QtWidgets import QFrame, QGridLayout, QLineEdit, QLabel, QWidget, QCheckBox, QComboBox
...
class AutoMethod(QFrame):
    ...
    def _make_widgets(self) -> None:
        for widget_name, specifier in self.input_widget_specifiers.items():
            if specifier.widget_type is WidgetType.TEXT:
                widget = QLineEdit(str(specifier.init_value if specifier.init_value else ""), validator=specifier.text_validator, parent=self)
                if specifier.range:
                    widget.editingFinished.connect(partial(self.validate_range, widget_name, specifier.range))
    ...
    def _make_layout(self) -> QGridLayout:
        ...
        if len(self.input_widgets) == 0:
            layout.addWidget(QLabel("No parameters to be set.", parent=self), grid_row + 1, 0)
        else:
