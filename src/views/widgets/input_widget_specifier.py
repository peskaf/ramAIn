...
class InputWidgetSpecifier:
    ...
    def __init__(
        ...
        if output_type not in self.NUMERIC_TYPES and range is not None:
            raise RuntimeError(
                f"Invalid InputWidgetSpecifier - range is set but output type is of type {output_type}."
            )
        ...
        if init_value is not None and not isinstance(init_value, output_type):
            if isinstance(init_value, int) and output_type is float:
                ...
                raise RuntimeError(
                    f"Invalid InputWidgetSpecifier - init value is of type {type(init_value)} but output type is {output_type}."
                )

        if widget_type is not WidgetType.COMBO_BOX and choices:
            raise RuntimeError(
                f"Invalid InputWidgetSpecifier - choices specified for not-combo-box widget type."
            )

        if widget_type is WidgetType.COMBO_BOX and not choices:
            raise RuntimeError(
                f"Invalid InputWidgetSpecifier - choices not specified for combo-box widget type."
            )

        if widget_type is WidgetType.COMBO_BOX and init_value:
            if init_value not in choices:
                raise RuntimeError(
                    f"Invalid InputWidgetSpecifier - init value not in choices for choice-type widget."
                )
            else:
                init_item_index = choices.index(init_value)
                choices[init_item_index], choices[0] = (
                    choices[0],
                    choices[init_item_index],
                )

        if widget_type is not WidgetType.TEXT and text_validator:
            raise RuntimeError(
                f"Invalid InputWidgetSpecifier - choices specified for non-combo-box widget type."
            )

        if widget_type is not WidgetType.DIRECTORY_SELECTION and dir_registry_value:
            raise RuntimeError(
                f"Invalid InputWidgetSpecifier - dir registry value specified for non-dir-selection widget type."
            )
